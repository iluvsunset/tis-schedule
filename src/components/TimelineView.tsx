  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      >
        ...
        <span className="px-2.5 py-1 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold border border-slate-200/80 dark:border-slate-700/80">
      ...
      <AnimatePresence mode="wait">
        <motion.div 
          ...
          className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6 items-start"
        >
          
          {/* SÁNG / MORNING COLUMN */}
          <div className="space-y-2.5 sm:space-y-3">
            <div className="flex items-center gap-2 px-1 text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200">
            ...
            <div className="space-y-2 sm:space-y-2.5">
              {morningItems.length === 0 ? (
                <div className="py-8 text-center text-xs sm:text-sm text-slate-400 dark:text-slate-500 glass-card rounded-2xl">
          ...
          <div className="space-y-2.5 sm:space-y-3">
            ...
            <motion.div 
              ...
              className={`py-2.5 px-4 rounded-2xl border text-xs sm:text-sm flex items-center justify-between transition-all ${
                lunchStatus.isCurrent 
                  ? 'bg-amber-500/10 dark:bg-amber-500/15 border-amber-400/50 text-amber-900 dark:text-amber-300 ring-2 ring-amber-400/40 shadow-sm' 
                  : lunchStatus.isPast 
            ...
            >
              <div className="flex items-center gap-2.5">
                <LunchIcon className="w-4 h-4 text-slate-500 dark:text-slate-400" />
                <span className={`font-semibold ${lunchStatus.isPast ? 'line-through decoration-slate-400 dark:decoration-slate-600 text-slate-400' : ''}`}>
                  {language === 'vi' ? 'Nghỉ trưa & Dùng bữa' : 'Lunch Break & Rest'}
                </span>
                {lunchStatus.isCurrent && (
                  <span className="text-[10px] sm:text-xs font-bold text-amber-700 dark:text-amber-400 bg-amber-100 dark:bg-amber-950/60 px-2 py-0.5 rounded-md border border-amber-200 dark:border-amber-800">
                    {language === 'vi' ? `Đang nghỉ (${lunchStatus.remainingMinutes}p)` : `Break (${lunchStatus.remainingMinutes}m)`}
                  </span>
                )}
              </div>
              <span className={`font-mono font-medium text-xs sm:text-sm ${lunchStatus.isPast ? 'line-through decoration-slate-400 dark:decoration-slate-600 text-slate-400' : 'text-slate-600 dark:text-slate-400'}`}>
                11:30 - 13:30
              </span>
            </motion.div>

            <div className="flex items-center gap-2 px-1 text-xs sm:text-sm font-bold text-slate-800 dark:text-slate-200 pt-0.5">
              <Sunset className="w-4 h-4 text-slate-500 dark:text-slate-400" />
